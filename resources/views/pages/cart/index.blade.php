...
@section('page_content')
    ...
    @if($cartItems->isEmpty())
        <div class="text-center py-12">
            ...
            <a href="{{ route('products.index') }}" class="mt-4 inline-block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">
                Continue Shopping
            </a>
        </div>
    @else
                ...
                <div class="flex flex-col sm:flex-row gap-4 p-4 border rounded-lg shadow-sm">
                    ...
                    <div class="flex-1">
                        <div class="flex justify-between">
                            ...
                            <p class="font-semibold">Rp. {{ number_format($item->product->price * $item->quantity) }}</p>
                        </div>
                        ...
                        <div class="mt-3 flex items-center">
                            ...
                            <span class="mx-2 px-3 py-1 border rounded text-center w-12">{{ $item->quantity }}</span>
                    ...
                    <form action="{{ route('cart.remove', $item->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="self-start sm:self-center p-2 text-red-500 hover:text-red-700">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
